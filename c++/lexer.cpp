    ...
    for (int k = 0; k < code.length(); k++) {
        ...
        if (i == " " or i == "\n") {
            ...
            for (auto j : data_types) {
                if (std::string(current_string.begin(), current_string.end()) == j) {
                    all_tokens.push_back(Token("datatype", std::string(current_string.begin(), current_string.end())));
                    current_string.clear();
                    continue;
                }
            }
            if (current_string.size() != 0) {
                all_tokens.push_back(Token("string", std::string(current_string.begin(), current_string.end())));
                current_string.clear();
            }
            if (current_number.size() != 0) {
                all_tokens.push_back(Token("number", std::string(current_number.begin(), current_number.end())));
